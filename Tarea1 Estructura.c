...
void codificar(char texto[], char str1[], char str2[], char nombreS[]){
  ...
  for(int i=0;i<strlen(texto);i++){
    ...
    for (int e=0;e<strlen(str1);e++){
      ...
      char p1=str1[e];
      char p2=str2[e];
      //si la letra del texto seleccionada coincide con una de las letras de las palabras claves.
      if(tolower(p1)==tolower(ct)){
...
void enigma(char nombreE[], char accion[], char palabra1[], char palabra2[], char nombreS[]) {
  ...
  if (corroborar==0){
    ...
    if (valor==0){
      printf("Se ha codificado el mensaje");
      leer(nombreE, palabra1, palabra2, nombreS);
      }
    if (valor2==0){
      printf("Se ha decodificado el mensaje");
      leer(nombreE, palabra1, palabra2, nombreS);
      }
    if (valor!=0 && valor2!=0) {
...
int main(void) {  
  ...
  char accion[30];
  printf("Que desea, codificar(encode) o decodificar(decode): ");
  scanf("%s", accion);
